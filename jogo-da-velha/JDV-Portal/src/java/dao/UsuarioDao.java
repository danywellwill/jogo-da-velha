    ...
    public List<Usuario> read(Integer cdUsuario) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                LocalDateTime dataRecup = aData == null ? null : LocalDateTime.parse(aData.toString(), DateTimeFormatter.ofPattern("yyyy-MM-DD HH:mm:ss"));
                usuario.setDtNascimento(dataRecup);
                usuario.setDeLogin(rs.getString("de_login"));
                usuario.setDeSenhaAcesso(rs.getString("de_senha_acesso"));
                aData = rs.getDate("dt_criacao");
                dataRecup = aData == null ? null : LocalDateTime.parse(aData.toString(), DateTimeFormatter.ofPattern("yyyy-MM-DD HH:mm:ss"));
                usuario.setDtCriacao(dataRecup);
                aData = rs.getDate("dt_alteracao");
                dataRecup = aData == null ? null : LocalDateTime.parse(aData.toString(), DateTimeFormatter.ofPattern("yyyy-MM-DD HH:mm:ss"));
                usuario.setDtAlteracao(dataRecup);
